...
def update_q_table(state, action, cost, next_state, q_table_loc, update_counts_loc):
    ...
    # learning_rate = 0.15 # Wegen deterministischer Umgebung?
...
for episode in range(NUM_EPISODES):
    ...
    for step in range(MAX_STEPS):
        epsilon = (1 / math.log(episode + 15))  # Decrease exploration rate over time
...
while state != GOAL_STATE:
    ...
    if action == 'N':
        next_state = (state[0], state[1] - 2) if state[1] > 0 else state
    elif action == 'S':
        next_state = (state[0], state[1] + 2) if state[1] < GRID_SIZE - 1 else state
    elif action == 'W':
        next_state = (state[0] - 2, state[1]) if state[0] > 0 else state
    elif action == 'O':
        next_state = (state[0] + 2, state[1]) if state[0] < GRID_SIZE - 1 else state
